...
const editAvatarFormValidation = new FormValidator (formConfigSelector, formElementChangeAvatar);
editAvatarFormValidation.enableValidation();

const popupImg = new PopupPicture(popupSelector.popupZoomImgContainer, popupSelector);
...
const popupUserAvatar = new PopupWithForm(popupSelector.popupAvatar, popupSelector, handleAvatarSubmit);
popupUserAvatar.setEventListeners();
const newUserInfo = new UserInfo(userDataSelector);
const popupConfirmDel = new PopupConfirmDel(popupSelector.popupConfirm, popupSelector, handleDeleteCard);
...
function handleDeleteCard(cardId, cardElement) {
  api.delCard(cardId, cardElement)
    .then(() => cardElement.remove())
    .then(() => popupConfirmDel.close())
    .catch((err) => console.log(err))
}

function handleLikeClick(likedBefore, likesArray, cardId, likeButton, buttonActiveSelector, likesCounter) {
    const currentUser = newUserInfo.getUserInfo()
    if(!likedBefore) {
      api.addLike(cardId, currentUser)
        .then((card) => {
      ...
      api.delLike(cardId, currentUser)
        .then((card) => {
